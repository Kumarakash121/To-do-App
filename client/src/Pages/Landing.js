...
const Landing = () => {
    ...
    const loadTodos = async () => {
        const response = await axios.get('https://to-do-app-cjw6.onrender.com/todo/gettodos', {
    ...
    const handleTaskToggle = async (todoId, taskName) => {
        try {
            const response = await axios.put(`https://to-do-app-cjw6.onrender.com/todo/updatetask/${todoId}`, {
            ...
            if (response.data.success) {
                ...
                // Refresh todos after updating task status
                loadTodos();
            } else {
            ...
            toast.error('Failed to mark task as done', { position: "top-right" });
        }
    };
    const submitData = async () => {
        ...
        try {
            ...
            const res = await axios.post('https://to-do-app-cjw6.onrender.com/todo/createtodo', data, {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
            })
    ...
    const handleDelete = async (id) => {
        try {
            const res = await axios.delete(`https://to-do-app-cjw6.onrender.com/todo/deletetodo/${id}`, {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
            })
            const response = await res.data
            if (response.success) {
    ...
    const handleEdit = async (id) => {
        try {
            ...
            settodo(newtodo)
            const res = await axios.put(`https://to-do-app-cjw6.onrender.com/todo/edittodo/${id}`, {
                "title": newtodo
            }, {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
            })
    ...
    const addTask = async (e) => {
        try {
            ...
            const taskData = {
                name: task, // Assuming `task` is the name of the task you want to add
                completed: false // Default value for completed status
            };
    
            const res = await axios.post(`https://to-do-app-cjw6.onrender.com/todo/createtask/${taskid}`, { task: taskData }, {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
            });
    ...
    const handleTaskClick = async (id) => {
        ...
        const res = await axios.get(`https://to-do-app-cjw6.onrender.com/todo/gettodo/${id}`, {
            headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
        })
    ...
    const handleTaskDelete = async (todoid,taskId) => {
        try {
            // e.preventDefault();
            ...
            const res = await axios.delete(`https://to-do-app-cjw6.onrender.com/todo/deletetask/${todoid}`, {
    ...
    const sort = async (n) => {
        const response = await axios.get('https://to-do-app-cjw6.onrender.com/todo/sortTodo', {
            params: { order: n },
            headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
        })
        ...
        <>
            ...
            <header className="p-4 bg-gray-100 text-gray-800">
                <div className="container flex flex-col md:flex-row text-center space-y-2 md:space-y-0 justify-between h-16 mx-auto">
                    <h3 className='font-semibold text-sm align-middle text-slate-700'>Welcome, {username}</h3>
                    ...
                    <table className="min-w-full text-xs">
                        ...
                        <thead className="bg-gray-300">
                            <tr className="text-left">
                                <th className="p-3 text-sm">Todo Names</th>
                                <th className="p-3 text-sm">Created At</th>
                                <th className="p-3 text-sm">Updated At</th>
                                <th className="p-3 text-sm">Tasks</th>
                                <th className="p-3 text-sm">Delete</th>
                                <th className="p-3 text-sm">Edit</th>
                            </tr>
                        </thead>
                        <tbody>
                            {
            ...
            </div>
            {
                showtask ? <div className="conatiner p-2 mx-auto sm:p-4 text-gray-800">
                    <h1 className="mb-4 text-2xl font-semibold leading-tight">TODO: {title} [Sub Task List]</h1>
                    <header className="p-4 bg-gray-100 text-gray-800">
                        <div className="container flex justify-between h-16 mx-auto">
                            <form onSubmit={addTask} className="flex items-center md:space-x-4 mx-auto">
                                ...
                                <button onClick={(e)=>handleTaskClick(taskid)} type="submit" className="px-6 py-2 font-semibold rounded lg:block bg-indigo-600 text-gray-50">Add Task</button>
                            </form>
                        </div>
                    </header>
                    <div className="overflow-x-auto shadow-md">
                                ...
                                <tbody>
                                    {
                                        tasks.map(task => (
                                          ...
                                          <tr tr key={todo._id} className="text-left" >
                                          <td className="p-3">
                                            <button
                                                className={`px-3 py-1 font-semibold rounded-md ${task.completed ? 'bg-green-500' : 'bg-gray-500'} text-white`}
                                                onClick={() => {
                                                    handleTaskToggle(taskid, task);
                                                    handleTaskClick(taskid);
                                                }}
                                            >
                                            ...
                                            {/* <label>{task.task}</label> */}
                                                ...
                                                <td className="p-3 ">
                                                    <button onClick={(e) =>{ 
                                                        handleTaskDelete(taskid,task._id)
